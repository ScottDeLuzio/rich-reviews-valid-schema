<?php
/*
Plugin Name: Rich Reviews - Valid Schema
Description: Rich Reviews empowers you to easily capture user reviews and display them on your WordPress page or post and in Google Search Results with valid Schema.org markup.
Version: 1.0
Author: Scott DeLuzio
Author URI: https://surpriseazwebservices.com/
Text Domain: rich-reviews-valid-schema
License: GPL2
	...
	*/
	...
	function shortcode_reviews_snippets($atts) {
		...
		// }
			...
			$output .= 'Overall rating: <span itemprop="ratingValue">';
			$output .= '<span class="stars">' . $stars . '</span>';
			$output .= '<span class="rating" itemprop="ratingValue" style="display: none !important;">' . $averageRating . '</span></span>';
			...
			$output = '<div itemprop="aggregateRating" itemscope itemtype="http://schema.org/AggregateRating">';
			$output .= 'Overall rating: <span itemprop="ratingValue">';
			$output .= '<strong><span class="value" itemprop="ratingValue">' . $averageRating . '</span></strong> out of <strong>5</strong> ';
	...
	function display_review($review) {
		...
		if($this->rr_options['display_full_width'] != NULL) {
			...
			if( $rTitle != '') {
				...
				$output .= '<h3 class="rr_title" itemprop="name" style="display:none">' . $rTitle . '</h3>';
			}
			$output .= '<div class="clear"></div>';
		} else {
		...
		if ($this->rr_options['show_date']) {
			if($rDateTime != "0000-00-00 00:00:00") {
				$output .= '<span class="rr_date">Submitted: <time itemprop="datePublished" datetime="' . $rDate . '">' . $rDate . '</time></span>';
			} else {
		...
		$output .= '<span itemprop="reviewRating" itemscope itemtype="http://schema.org/Rating"><div class="stars"><meta itemprop="ratingValue" content="' . $rRatingVal . '" />' . $rRating . '</div><div style="display:none;" itemprop="bestRating">' . $rRatingVal . '</div></span>';
		...
		if( $rName !='' ) {
			$output .= '<div class="rr_review_name"> - <span itemprop="author">' . $rName . '</span></div>';
		}
		$output .=	'<div class="clear"></div>';
		$output .= '</div>';
		return __($output, 'rich-reviews');
